...
class API {
  ...
  private _stateListeners: Map<
    string,
    { id: number; callback: IStateCallback }[]
  > = new Map();
   ...
   * @returns {Promise<IState>}    Promise with the state
   * @memberof API
   */
  ...
  private _onStateChange() {
    ...
    this._connection
      .subscribeEvent('state_changed', message => {
        const newState: IState = message.data.new_state;
        if (!newState) {
          return;
        }
        Logger.debug(
          `New state of ${newState.attributes.friendly_name} (${newState.entity_id}): ${newState.state}`,
        );
  ...
  private async _bootstrap(): Promise<void> {
    const automationsDir: string = path.resolve(
